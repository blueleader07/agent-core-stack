# Bedrock Agents Example

Traditional AWS Bedrock Agents (Infrastructure-as-Code) with action groups, WebSocket streaming, and Firebase authentication.

> **Note:** This uses the traditional **Bedrock Agents** service (deployed via CDK as infrastructure). For the newer **AgentCore Runtime** service with ADOT observability and containerized agents, see the [agent-core example](../agent-core/).

## What is This?

This example demonstrates a **Bedrock Agent** using AWS's managed agent service (infrastructure-based). The agent has:
